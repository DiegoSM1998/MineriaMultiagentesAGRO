...
muestra = processedTNacional[mask]
muestra = muestra.assign(Prediccion_Volumen=y_pred[:,0])
muestra = muestra.assign(P_precio=y_pred[:,2])
muestra = muestra.assign(P_penetración=y_pred[:,3])
patatas = muestra[muestra['Producto_TOTAL PATATAS'] == 1]
print(muestra)


patatas.plot(y=["Volumen (miles de kg)","Prediccion_Volumen"], kind='line')
plt.savefig('regression_AMPLImodelpatatas.png')
plt.show()
plt.clf()
patatas.plot(y=["Penetración (%)","P_penetración"], kind='line')
plt.savefig('penetraciónPatatas.png')
plt.show()
plt.clf()
patatas.plot(y=["Precio medio kg","P_precio"], kind='line')
plt.savefig('precioMedioPatatas.png')
plt.show()

muestra['DiferenciaVolumenEsperado'] = muestra['Volumen (miles de kg)'] - muestra['Prediccion_Volumen']

product_type_columns = ['Producto_AJOS', 'Producto_ALCACHOFAS', 'Producto_APIO',
       'Producto_BERENJENAS', 'Producto_BROCOLI', 'Producto_CALABACINES',
       'Producto_CEBOLLAS', 'Producto_CHAMPIÑONES+O.SETAS', 'Producto_COLES',
       'Producto_COLIFLOR', 'Producto_ESPARRAGOS', 'Producto_JUDIAS VERDES',
       'Producto_LECHUGA/ESC./ENDIVIA', 'Producto_OTR.HORTALIZAS/VERD.',
...
diferenciaVol = pd.DataFrame(columns=['Producto','Diferencia media real-esperada'])

for col in product_type_columns:
    type_mean = muestra[muestra[col]==1]['DiferenciaVolumenEsperado'].mean()
    diferenciaVol = diferenciaVol.append({'Producto':col,'Diferencia media real-esperada': type_mean}, ignore_index=True)
plt.clf()
plt.figure(figsize=(15, 15))
plt.bar(diferenciaVol['Producto'], diferenciaVol['Diferencia media real-esperada'])
plt.xticks(rotation=80)
plt.savefig('Diferencia.png')
plt.show()
#############################################################################
muestra['DiferenciaPenetracion'] = muestra["Penetración (%)"] - muestra["P_penetración"]

diferenciaPe = pd.DataFrame(columns=['Producto','Diferencia media real-esperada'])

for col in product_type_columns:
    type_mean = muestra[muestra[col]==1]['DiferenciaPenetracion'].mean()
    diferenciaPe = diferenciaPe.append({'Producto':col,'Diferencia media real-esperada': type_mean}, ignore_index=True)
plt.clf()
plt.figure(figsize=(15, 15))
plt.bar(diferenciaPe['Producto'], diferenciaPe['Diferencia media real-esperada'])
plt.xticks(rotation=80)
plt.savefig('DiferenciaPenetración.png')
plt.show()
